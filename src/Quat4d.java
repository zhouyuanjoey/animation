...
public class Quat4d extends Tuple4d implements java.io.Serializable {
 ...
 /**
  ...
  public final void normalize()
  {
     ...
     */
    public final void setAxisAngle(double ax, double ay, double az, double aangle)
    {
	double mag,amag;
	...
	if( amag < EPS ) {
	    ...
	    amag = 1.0/amag; 
	    mag = Math.sin(aangle/2.0);
	    w = Math.cos(aangle/2.0);
       x = ax*amag*mag;
       y = ay*amag*mag;
       z = az*amag*mag;
	}
    ...
    //returns the axis followed by the angle
    public double[] getaxisangle(){
    	double rtw = Math.sqrt(1-w*w) ;
    	return new double[]{x / rtw ,y / rtw ,z / rtw, 2 * Math.acos(w)} ;
    }
